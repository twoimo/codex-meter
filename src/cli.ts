...
import { loadConfig, type MeterConfig } from './config.js';
...
import { decide, worstSeverity, type PolicyOutcome, type PullRequestMeta } from './policy.js';
import { priceFor, costUsd, roundUsd } from './pricing.js';
import { resolvePrompt } from './prompt.js';
import { renderErrorComment, renderReportMarkdown, renderReviewComment, renderSkipComment, type SpendLine } from './render.js';
import { EMPTY_USAGE, SEVERITY_RANK, type LedgerRecord, type Severity, type Usage } from './types.js';
 ...
 */
function logCommentResult(result: 'created' | 'updated' | 'failed'): void {
  if (result === 'failed') {
...
function numberOption(options: CliOptions, key: string): number | null {
...
function stringOption(options: CliOptions, key: string): string | null {
  const raw = options[key];
  return typeof raw === 'string' && raw.length > 0 ? raw : null;
}

async function resolveBase(cwd: string, requested: string | null, pull: PullDetails | null): Promise<string> {
  if (requested) return requested;
  if (pull?.baseRef) {
    const remote = await revParse(cwd, `origin/${pull.baseRef}`);
    if (remote) return `origin/${pull.baseRef}`;
    const local = await revParse(cwd, pull.baseRef);
    if (local) return pull.baseRef;
  }
  return defaultBase(cwd);
}

function clientConfigWithOverrides(config: MeterConfig, options: CliOptions): MeterConfig {
  const next: MeterConfig = structuredClone(config);

  const model = stringOption(options, 'model');
  if (model) next.model = model;

  const codexBin = stringOption(options, 'codexBin');
  if (codexBin) next.codexBin = codexBin;
  ...
  if (budgetTokens !== null) next.budget.tokensPerMonth = budgetTokens;

  const budgetUsd = numberOption(options, 'budgetUsd');
  if (budgetUsd !== null) next.budget.usdPerMonth = budgetUsd;

  const runTokens = numberOption(options, 'runTokens');
  if (runTokens !== null) next.budget.tokensPerRun = runTokens;

  const maxChangedLines = numberOption(options, 'maxChangedLines');
  if (maxChangedLines !== null) next.maxChangedLines = maxChangedLines;

  const minChangedLines = numberOption(options, 'minChangedLines');
  if (minChangedLines !== null) next.minChangedLines = minChangedLines;

  const state = stringOption(options, 'state');
  if (state === 'file' || state === 'branch') next.state.mode = state;
  ...
  if (comment === 'upsert' || comment === 'off') next.comment = comment;
  if (options['noComment'] === true) next.comment = 'off';

  const failOn = stringOption(options, 'failOn');
  if (failOn === 'never' || (failOn !== null && ['critical', 'high', 'medium', 'low', 'info'].includes(failOn))) {
    next.failOn = failOn as MeterConfig['failOn'];
  }

  if (options['allowForkPrs'] === true) next.allowForkPrs = true;
  if (options['dryRun'] === true) next.dryRun = true;
  if (options['respectUserConfig'] === true) next.ignoreUserConfig = false;

  return next;
}

function createBranchStore(ctx: RepoContext, branch: string, filePath: string, baseHint: string | null): LedgerStore {
...
function emptyFindings(): Record<Severity, number> {
  return { critical: 0, high: 0, medium: 0, low: 0, info: 0 };
}

function countFindings(usageFindings: { severity: Severity }[]): Record<Severity, number> {
  const counts = emptyFindings();
  for (const finding of usageFindings) counts[finding.severity] += 1;
  return counts;
}

async function commandReview(options: CliOptions): Promise<number> {
  ...
  if (unknownKeys.length > 0) log(`warning: unknown config keys ignored: ${unknownKeys.join(', ')}`);
  ...
  if (!pull && ctx && explicitPr !== null) pull = await fetchPull(ctx, explicitPr);
  if (!pull && ctx && env.eventPath) log('warning: no pull_request payload found in the event; continuing without PR metadata');
  ...
  try {
    ...
    log(`warning: could not read ledger (${store.describe()}): ${(error as Error).message}`);
    records = [];
  }
  ...
  const outcome: PolicyOutcome = decide({
    config,
    stats,
  ...
  const spendLine = (usage: Usage, cost: number | null, model: string | null): SpendLine => ({
    usage,
    costUsd: cost,
    model,
  ...
  log(
    `github: repo=${repoSlug ?? 'none'} pull=${pull?.number ?? explicitPr ?? 'none'} comment=${config.comment} token=${env.token ? 'present' : 'missing'}`,
  );
  ...
  for (const note of outcome.decision.notes) log(`note: ${note}`);

  if (!outcome.decision.run || config.dryRun) {
    if (config.dryRun) log('dry run: no Codex run, no comment, no ledger write');
    ...
    if (!config.dryRun && outcome.decision.run === false) {
      ...
      try {
        ...
        log(`warning: could not write ledger: ${(error as Error).message}`);
      }
    }
  ...
  if (apiKey) {
    ...
    if (!login.ok) {
      ...
      if (throwaway) await removeCodexHome(throwaway);
  ...
  if (result.errorKind !== 'none' && result.failureReason) log(`codex failure: ${result.failureReason}`);
  ...
  if (ctx && pull?.number && config.comment === 'upsert') {
    const posted =
      result.errorKind === 'none' && result.review
        ? await upsertComment(
            ctx,
            pull.number,
            renderReviewComment({
  ...
  if (result.errorKind !== 'none') return 0;
...
async function commandExplain(options: CliOptions): Promise<number> {
  const cwd = stringOption(options, 'cwd') ?? process.cwd();
  ...
  const payload = {
    ...
    stats: {
      ...
      byKind: stats.files.reduce<Record<string, number>>((acc, file) => {
...
async function commandReport(options: CliOptions): Promise<number> {
  const cwd = stringOption(options, 'cwd') ?? process.cwd();
  ...
  try {
    ...
    log(`error: could not read ledger (${store.describe()}): ${(error as Error).message}`);
    return 2;
  }
