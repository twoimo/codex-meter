...
export async function mergeBase(cwd: string, base: string, head: string): Promise<string | null> {
...
function parseNumstat(stdout: string): NumstatEntry[] {
  ...
  for (const line of stdout.split('\n')) {
    ...
    const [addedRaw, deletedRaw, ...pathParts] = parts as [string, string, ...string[]];
...
function parseNameStatus(stdout: string): Map<string, ChangedFile['status']> {
 ...
 */
export async function diffStats(cwd: string, base: string, head: string): Promise<DiffStats> {
  ...
  const statuses = nameStatus.ok ? parseNameStatus(nameStatus.stdout) : new Map<string, ChangedFile['status']>();
  const files: ChangedFile[] = parseNumstat(numstat.stdout).map((entry) => ({
    path: entry.path,
    added: entry.added,
