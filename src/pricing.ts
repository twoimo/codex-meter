...
export interface PriceMatch {
  model: string;
  price: ModelPrice;
  /** true when the requested model name was resolved through a shorter prefix */
  approximate: boolean;
}
...
export function priceFor(
  ...
  if (exact) return { model: key, price: exact, approximate: false };
  ...
  for (let take = parts.length - 1; take >= 2; take -= 1) {
    ...
    if (hit) return { model: candidate, price: hit, approximate: true };
  }
  return null;
}
...
const perMillion = (tokens: number, rate: number | null): number =>
  rate === null ? 0 : (Math.max(0, tokens) / 1_000_000) * rate;

export function costUsd(usage: Usage, price: ModelPrice): CostBreakdown {
