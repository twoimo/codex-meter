 ...
 */
export function providerArgs(provider: ProviderConfig, model: string | null): string[] {
  ...
  if (modelId) args.push('-c', `model=${tomlString(modelId)}`);
  return args;
}

export function providerCredentialPresent(provider: ProviderConfig | null): boolean {
