...
test('the full review path runs, meters and records a real session', async () => {
    ...
    const result = await runCli(['review', '--cwd', dir, '--base', 'main', '--codex-bin', stub, '--model', 'gpt-5.4', '--json'], dir, { STUB_PROMPT_OUT: promptOut });
...
test('the monthly budget stops reviews once it is spent', async () => {
    ...
    const stub = await installCodexStub();
    const result = await runCli(['review', '--cwd', dir, '--base', 'main', '--codex-bin', stub, '--budget-tokens', '1000', '--json'], dir);
    assert.equal(result.code, 0);
    const payload = JSON.parse(result.stdout);
