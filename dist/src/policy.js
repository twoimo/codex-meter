...
export function isBotAuthor(author) {
    ...
    if (name.endsWith('[bot]'))
        return true;
    if (name.startsWith('app/') && name.length > 'app/'.length)
        return true;
...
export function classifyPath(filePath) {
    ...
    if (GENERATED_PATTERNS.some((pattern) => pattern.test(normalized)))
        return 'generated';
    if (DOCS_PATTERNS.some((pattern) => pattern.test(normalized)))
        return 'docs';
    if (/^\.github\/workflows\//i.test(normalized) || /^(Dockerfile|docker-compose.*)$/i.test(normalized))
        return 'ci';
...
export function breakDown(stats) {
    const byKind = { docs: 0, lock: 0, generated: 0, test: 0, ci: 0, source: 0 };
...
export function estimateTokens(stats, config) {
...
export function estimateUsd(estimate, model, pricing) {
...
export function decide(input) {
    ...
    if (stats.fileCount === 0)
        return finish(false, 'no-changes', 'No changed files found between base and head.');
    ...
    if (config.skipGeneratedOnly && breakdown.generatedOnly) {
        return finish(false, 'generated-only', `Only generated or lock files changed (${stats.fileCount} files: ${breakdown.byKind.generated} generated, ${breakdown.byKind.lock} lock).`);
    }
    if (stats.changedLines < config.minChangedLines) {
        return finish(false, 'too-small', `Diff has ${stats.changedLines} changed lines, below minChangedLines=${config.minChangedLines}.`);
    }
    if (stats.changedLines > config.maxChangedLines) {
        return finish(false, 'too-large', `Diff has ${stats.changedLines} changed lines, above maxChangedLines=${config.maxChangedLines}. Split the pull request or raise the limit.`);
    }
    ...
    if (config.budget.tokensPerMonth !== null && monthTokens > config.budget.tokensPerMonth) {
        return finish(false, 'budget-exhausted', `Monthly token budget reached: ${budget.monthTokens} spent, ~${estimate.input + estimate.output} estimated for this run, limit ${config.budget.tokensPerMonth}.`);
    }
    if (config.budget.tokensPerRun !== null && estimate.input + estimate.output > config.budget.tokensPerRun) {
        return finish(false, 'budget-exhausted', `Estimated run cost (~${estimate.input + estimate.output} tokens) exceeds tokensPerRun=${config.budget.tokensPerRun}.`);
    }
    if (config.budget.usdPerMonth !== null) {
        if (estimateCost === null || budget.monthUsd === null) {
            ...
            return finish(false, 'budget-exhausted', `Monthly dollar budget reached: ${usd(budget.monthUsd)} spent, ~${usd(estimateCost)} estimated, limit ${usd(config.budget.usdPerMonth)}.`);
        }
    }
    if (config.budget.usdPerRun !== null && estimateCost !== null && estimateCost > config.budget.usdPerRun) {
        return finish(false, 'budget-exhausted', `Estimated run cost (~${usd(estimateCost)}) exceeds usdPerRun=${usd(config.budget.usdPerRun)}.`);
    }
