...
async function manifest(): Promise<Record<string, any>> {
  ...
  const parsed = parse(text) as Record<string, any> | null;
...
test('action.yml is valid YAML and declares the expected entry point', async () => {
  ...
  assert.ok((parsed['description'] as string).length > 20, 'description should say what the action does');
  assert.equal(parsed['runs']?.['using'], 'composite');

  const steps = parsed['runs']?.['steps'] as Record<string, unknown>[] | undefined;
  ...
  assert.ok(
    runSteps.some((step) => /dist\/src\/cli\.js/.test(String(step['run']))),
    'one step must execute the built CLI (dist/src/cli.js)',
  );
  assert.ok(
    steps.some((step) => /^actions\/setup-node@v\d+$/.test(String(step['uses']))),
    'the action installs Node itself',
  );
});
...
test('every input and output is documented', async () => {
  ...
  for (const [name, input] of Object.entries(inputs)) {
    ...
    assert.ok((input['description'] as string).trim().length > 0, `input ${name} needs a description`);
    if ('required' in input) assert.equal(typeof input['required'], 'boolean', `input ${name} required must be a boolean`);
  }
