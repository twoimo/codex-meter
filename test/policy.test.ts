...
import type { DiffStats } from '../src/types.js';
import type { PolicyInput } from '../src/policy.js';

function stats(entries: [string, number, number][]): DiffStats {
...
test('cheap gates run before expensive ones', () => {
  const draft = decide(
    input({ pull: { number: 1, title: null, draft: true, author: null, labels: [], isFork: false } }),
  );
  assert.equal(draft.decision.code, 'draft');

  const labelled = decide(
    input({ pull: { number: 1, title: null, draft: false, author: null, labels: ['skip-codex-meter'], isFork: false } }),
  );
  assert.equal(labelled.decision.code, 'label-skip');

  const fork = decide(
    input({ pull: { number: 1, title: null, draft: false, author: 'stranger', labels: [], isFork: true } }),
  );
...
test('automation accounts are recognised and skipped without spending', () => {
  ...
  const skipped = decide(
    input({ pull: { number: 9, title: null, draft: false, author: 'dependabot[bot]', labels: [], isFork: false } }),
  );
...
test('metadata gates keep their documented precedence', () => {
  ...
  assert.equal(
    decide(input({ pull: { ...stacking, draft: false, author: 'human' } })).decision.code,
    'label-skip',
  );
  assert.equal(
    decide(input({ pull: { ...stacking, draft: false, author: 'human', labels: [] } })).decision.code,
    'fork-pr',
  );
  ...
  optOut.skipBotAuthors = false;
  assert.equal(
    decide(input({ config: optOut, pull: { ...stacking, draft: false } })).decision.code,
    'label-skip',
    'the label gate still wins once bot gating is disabled',
  );
  assert.equal(
    decide(input({ config: optOut, pull: { ...stacking, draft: false, labels: [] } })).decision.code,
    'fork-pr',
    'the fork gate still wins once bot gating is disabled',
  );
});
