...
function run(args: string[], cwd: string): Promise<{ code: number; stdout: string; stderr: string }> {
  return new Promise((resolve) => {
    execFile(
      process.execPath,
      [CLI, ...args],
      { cwd, env: { ...process.env, CODEX_API_KEY: 'test-key', GITHUB_TOKEN: '', GITHUB_REPOSITORY: '', GITHUB_EVENT_PATH: '' } },
      (error, stdout, stderr) => {
...
async function fixtureRepo(files: [string, string][]): Promise<string> {
